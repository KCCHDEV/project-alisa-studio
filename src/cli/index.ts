...
async function main() {
  ...
  const model = process.env.ICHIGO_MODEL || 'deepseek/deepseek-chat';
  ...
  const agent = new Agent({
    ...
    onEvent: (event: AgentEvent) => {
      if (event.type === 'token_stream') {
        ...
        console.log(`\n\x1b[36m⚙️  Executing [${event.toolName}]...\x1b[0m \x1b[90m${JSON.stringify(event.args)}\x1b[0m`);
      } else if (event.type === 'tool_call_end') {
        if (event.error) {
          console.log(`\x1b[31m❌ Tool Error:\x1b[0m ${event.error}`);
        } else {
          console.log(`\x1b[32m✅ Tool Finished\x1b[0m`);
        }
  ...
  const askPrompt = () => {
    rl.question('\n\x1b[38;5;198m🍓 Ichigo > \x1b[0m', async (input) => {
      ...
      if (trimmed.toLowerCase() === 'exit' || trimmed.toLowerCase() === 'quit') {
        console.log('\n\x1b[32mบ๊ายบายค่ะคุณ Hori! 🍓\x1b[0m');
        rl.close();
        process.exit(0);
      }
